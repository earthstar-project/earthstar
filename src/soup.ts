...
type WillMatch = 'all' | 'all-latest' | 'some' | 'nothing';
interface CleanUpQueryResult {
    query: Query,
    isValid: boolean,
    willMatch: WillMatch;
}
let cleanUpQuery = (inputQuery: Query): CleanUpQueryResult => {
    ...
    // valid enum values
    if (query.history !== undefined && query.history !== 'all' && query.history !== 'latest') { return invalidResponse; }
    ...
    let willMatch: WillMatch = query.history === 'all' ? 'all' : 'all-latest';
