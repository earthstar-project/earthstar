...
export let cleanUpQuery = (inputQuery: Query): CleanUpQueryResult => {
  ...
  if (query.orderBy !== undefined) {
    if (
      ["path ASC", "path DESC", "localIndex ASC", "localIndex DESC"].indexOf(
        query.orderBy,
      ) === -1
    ) {
      logger.debug(
...
export let docMatchesFilter = (doc: Doc, filter: QueryFilter): boolean => {
  ...
  if (
    filter.timestampGt !== undefined && !(doc.timestamp > filter.timestampGt)
  ) {
    return false;
  }
  if (
    filter.timestampLt !== undefined && !(doc.timestamp > filter.timestampLt)
  ) {
    return false;
  }
  let contentLength = stringLengthInBytes(doc.content);
  if (
    filter.contentLength !== undefined && contentLength !== filter.contentLength
  ) {
    return false;
  }
  if (
