 ...
 */
export let queryByGlobAsync = async (storage: IStorageAsync, glob: string, moreQueryOptions: Query = {}): Promise<Doc[]> => {
  let { query, regex } = globToQueryAndRegex(glob);
 ...
 *      }
 */
export let queryByTemplateAsync = async (storage: IStorageAsync, template: string, moreQueryOptions: Query = {}): Promise<Doc[]> => {
