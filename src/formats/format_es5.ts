 ...
 */
export const FormatEs5: IFormat<"es.5", DocInputEs5, DocEs5, Es5Credentials> =
  class {
    static id: "es.5" = "es.5";
    ...
    static async generateDocument(
      { input, credentials, share, timestamp, prevLatestDoc }:
    ...
    > {
      ...
      const doc: DocEs5 = {
        ...prevLatestDoc,
        format: "es.5",
        author: credentials.authorKeypair.address,
        text: nextText,
      ...
      const signed = await this.signDocument(credentials, doc);
    ...
    static async signDocument(
      ...
      const sig = await Crypto.sign(credentials.authorKeypair, hash);

      const shareSig = await Crypto.sign({
        shareAddress: doc.share,
        secret: credentials.shareSecret,
      }, hash);
    ...
    static async wipeDocument(
      ...
      if (cleanedDoc.attachmentHash) {
        ...
        return this.signDocument(credentials, emptyDoc);
      }
    ...
    static updateAttachmentFields(
      ...
      return this.signDocument(credentials, updatedDoc);
    }

    static authorFromCredentials(credentials: Es5Credentials): AuthorAddress {
      return credentials.authorKeypair.address;
    }
  };
