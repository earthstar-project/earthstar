 ...
 * 
 * If the query has a limit or limitBytes, do multiple queries to
 * advance all the way through the documents.
 */
async function* localQueryIterator(storage: IStorage3, query: Query3): AsyncGenerator<Document, void, unknown> {
    while (true) {
        ...
        query = {
            ...query,
            //continueAfter: [lastDoc.path, lastDoc.author],  // TODO
        };
    }
}

/** Map a doc async-iterator to fingerprints. */
async function* docToFingerprintIterator(docs: AsyncGenerator<Document, void, unknown>): AsyncGenerator<Fingerprint, void, unknown> {
 ...
 */
async function* fingerprintIteratorZipper(fIter1: AsyncGenerator<Fingerprint, void, unknown>, fIter2: AsyncGenerator<Fingerprint, void, unknown>):
