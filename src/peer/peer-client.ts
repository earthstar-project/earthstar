...
import { IngestEvent } from "../storage/storage-types.ts";
import {
...
export class PeerClient implements IPeerClient {
    ...
    async setState(newState: Partial<PeerClientState>): Promise<void> {
        ...
        if (
            newState.serverPeerId !== null &&
            newState.serverPeerId !== undefined
        ) {
            if (this.state.serverPeerId !== null) {
                if (newState.serverPeerId !== this.state.serverPeerId) {
                    logger.warn(
                        `server has changed peer ID from ${this.state.serverPeerId} to ${newState.serverPeerId}; resetting PeerClient state`,
                    );
    ...
    async handle_saltyHandshake(
        ...
        loggerHandle.debug(
            "...salting and hashing my own workspaces and comparing with server...",
        );
        ...
        loggerHandle.debug(
            `...server has ${saltedWorkspaces.length} workspaces; we have ${this.peer.workspaces().length}; and ${commonWorkspaces.length} are in common`,
        );
    ...
    async do_allWorkspaceStates(): Promise<void> {
        ...
        if (
            this.state.commonWorkspaces === null ||
            this.state.commonWorkspaces.length === 0
        ) {
            loggerDo.debug(
                "...actually there are no common workspaces to ask about, so quitting early",
            );
            return;
        }
        ...
        loggerDo.debug("...client is going to handle_ ...");
        let stateUpdate = await this.handle_allWorkspaceStates(
            request,
            response,
        );
    ...
    async handle_allWorkspaceStates(
    ...
    async process_workspaceQuery(
        response: WorkspaceQuery_Response,
    ): Promise<number> {
        ...
        if (storageId !== myWorkspaceState.serverStorageId) {
            let err =
                `storageId for ${workspace} is not ${storageId} anymore, it's ${myWorkspaceState.serverStorageId}`;
            loggerProcess.error(err);
            throw err;
        }
        ...
        for (let doc of docs) {
            ...
            if (ingestEvent.kind === "failure") {
                ...
                loggerProcess.error(
                    "if it is invalid, it might be from the future;",
                );
