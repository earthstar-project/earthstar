...
export let runStorageDriverBenchmark = async (
    runner: BenchmarkRunner,
    cryptoDriver: ICryptoDriver,
    makeStorageDriver: () => IStorageDriverAsync,
    scenarioName: string,
) => {
    //==================================================
    // setup
    ...
    for (let n of [100, 500]) {
        let storageAdd = new StorageAsync(
            workspace,
            validator,
            makeStorageDriver(),
        );
        ...
        let storageSync = new StorageAsync(
            workspace,
            validator,
            makeStorageDriver(),
        );
        await runner.runOnce(`sync ${n} docs to empty storage (docs/sec)`, {
            actualIters: n,
        }, async () => {
        ...
        await runner.runOnce(
            `sync ${n} docs again to full storage (docs/sec)`,
            { actualIters: n },
            async () => {
                await syncLocal(storageAdd, storageSync);
            },
