...
export let runStorageDriverTests = (scenario: TestScenario) => {
  ...
  Deno.test(SUBTEST_NAME + ": empty storage, close", async () => {
    ...
    assertEquals(driver.getMaxLocalIndex(), -1, "maxLocalIndex starts at -1");
    assertEquals(await driver.queryDocs({}), [], "query returns empty array");
  ...
  Deno.test(SUBTEST_NAME + ": config", async () => {
    ...
    assertEquals(await driver.listConfigKeys(), [], `listConfigKeys() is []`);
    assertEquals(
  ...
  Deno.test(
    ...
    async () => {
      ...
      assertEquals(latestDocs[0]._localIndex, 2, "latestDocs[0]._localIndex");
      assertEquals(latestDocs[0].content, "Hello 2", "content is from doc2");
      ...
      assertEquals(allDocs[1].content, "Hello 1", "latestDocs[1].content is 1");
      ...
      assertEquals(latestDocs.length, 1, "there is 1 latest doc");
      assertEquals(latestDocs[0]._localIndex, 1, "latestDocs[0]._localIndex");
      assertEquals(latestDocs[0].content, "Hello 1", "content is from doc1");
      ...
      assertEquals(allDocs[1].content, "Hello 3", "latestDocs[1].content is 3");
      ...
      assertEquals(allDocs[1].content, "Hello 3", "latestDocs[1].content is 3");
      assertEquals(allDocs[2].content, "Hello 4", "latestDocs[2].content is 4");
      ...
      for (let { query, expectedContent } of vectors) {
        ...
        assertEquals(actualContent, expectedContent, `query: ${J(query)}`);
      }
