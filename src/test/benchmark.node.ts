...
interface PartsInput {
    ...
    storageDriverClass: ClassThatImplements<IStorageDriverAsync>;
}
interface Parts {
    description: string,
    cryptoDriver: ICryptoDriver;
    ...
    makeStorage: (workspace: WorkspaceAddress) => IStorageAsync;
}
...
let main = async () => {
    ...
    for (let scenario of scenarios) {
        ...
        await runner.runMany('crypto: sha256', {minDuration: 1234}, async () => {
            parts.crypto.driver.sha256('' + Math.random() + Math.random() + Math.random() + Math.random());
        });

        await runner.runMany('crypto: generateAuthorKeypair', {minDuration: 1234}, async () => {
            parts.crypto.generateAuthorKeypair('abcd');
        });

        let msg = 'hello';
        await runner.runMany('crypto: sign', {minDuration: 1234}, async () => {
            parts.crypto.sign(authors.keypair1, msg);
        });

        let sig = parts.crypto.sign(authors.keypair1, 'hello');
        await runner.runMany('crypto: validate', {minDuration: 1234}, async () => {
