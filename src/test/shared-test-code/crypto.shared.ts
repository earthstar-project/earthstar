...
export let runCryptoTests = (driver: ICryptoDriver) => {
  ...
  Deno.test(SUBTEST_NAME + ": generateAuthorKeypair", async () => {
    ...
    if (isErr(keypair)) {
      ...
      assertStrictEquals(typeof keypair.secret, "string", "keypair has secret");
      assert(
        keypair.address.startsWith("@ok99."),
        "keypair.address starts with @ok99.",
      );
      assert(keypair.secret.startsWith("b"), 'keypair.secret starts with "b"');
    }
  ...
  Deno.test(SUBTEST_NAME + ": signatures", async () => {
    ...
    assert(
      await Crypto.verify(keypair.address, snowmanStringSig, snowmanString),
