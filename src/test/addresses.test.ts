...
t.test('assemble addresses', (t: any) => {
...
t.test('parseAuthorAddress', (t: any) => {
    let vectors: AuthorAddressVector[] = [
        ...
        { valid: false, address: '@suzy@bxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx', note: '@ instead of .' },
        ...
        { valid: false, address: '@suzy.bxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx.extra', note: 'too many periods (and too long)' },
        { valid: false, address: '@suzy.bxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx.xxxxxxxx', note: 'too many periods (53 chars)' },
        ...
        { valid: false, address: '@suzy.7xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx', note: 'key starts with a number (53 chars)' },
    ...
    for (let v of vectors) {
        if (v.valid) {
            t.same(parseAuthorAddress(v.address), v.parsed, 'should be parsable: ' + (v.note || v.address));
            t.ok(notErr(checkAuthorIsValid(v.address)),    'should be valid:    ' + (v.note || v.address));
        } else {
            t.ok(isErr(parseAuthorAddress(v.address)),  'should be unparsable: ' + (v.note || v.address));
            t.ok(isErr(checkAuthorIsValid(v.address)), 'should be invalid:    ' + (v.note || v.address));
        }
    }
    t.end();
});
...
t.test('parseWorkspaceAddress', (t: any) => {
    ...
    for (let v of vectors) {
        if (v.valid) {
            t.same(parseWorkspaceAddress(v.address), v.parsed, 'should be parsable: ' + (v.note || v.address));
            t.ok(notErr(checkWorkspaceIsValid(v.address)),    'should be valid:    ' + (v.note || v.address));
        } else {
            t.ok(isErr(parseWorkspaceAddress(v.address)),  'should be unparsable: ' + (v.note || v.address));
            t.ok(isErr(checkWorkspaceIsValid(v.address)), 'should be invalid:    ' + (v.note || v.address));
        }
    }
    t.end();
});
